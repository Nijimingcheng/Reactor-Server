...
int main(int argc,char *argv[])
{
    ...
    servaddr.sin_addr.s_addr = inet_addr(argv[1]);      // 服务端用于监听的ip地址。inet_addr函数将点十分制的网络ip转换成二进制的网络ip。
    ...
    while (true)        // 事件循环。
    {
        ...
        for (int ii=0;ii<infds;ii++)       // 遍历epoll返回的数组evs。
        {
            if (evs[ii].data.fd==listenfd)
            {
                ...
                printf ("accept client(fd=%d,ip=%s,port=%d) ok.\n",clientfd,inet_ntoa(clientaddr.sin_addr),ntohs(clientaddr.sin_port)); //inet_ntoa函数是将二进制的网络ip转换成十分点进制的网络ip
            ...
            else                                        // 如果是客户端连接的fd有事件。
            {
                ...
                if (evs[ii].events & EPOLLRDHUP)  //第一种：对方已关闭，有些系统检测不到，可以使用EPOLLIN，recv()返回0。
                {
                    printf("client(eventfd=%d) disconnected. (第一种情况)\n",evs[ii].data.fd);
                    close(evs[ii].data.fd);            // 关闭客户端的fd。
                }                                //  普通数据  带外数据
                else if (evs[ii].events & (EPOLLIN|EPOLLPRI))   //第二种：接收缓冲区中有数据可以读。
                {
                    char buffer[1024];
                    while (true)             // 由于使用非阻塞IO，一次读取buffer大小数据，直到全部的数据读取完毕。
                    {
                        ...
                        {
                            printf("client(eventfd=%d) disconnected. (第二种情况)\n",evs[ii].data.fd);
                            close(evs[ii].data.fd);            // 关闭客户端的fd。
                            break;
                        }
                    }
                }
                else if (evs[ii].events & EPOLLOUT)  //第三种：有数据需要写，暂时没有代码，以后再说。
                {
                }
                else   //第四种：其它事件，都视为错误。
                {
                    printf("client(eventfd=%d) error. (第三种情况)\n",evs[ii].data.fd);
                    close(evs[ii].data.fd);            // 关闭客户端的fd。
                }
            }
        }
